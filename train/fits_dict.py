    ...
    def __init__(self, options, train_dataset):
        ...
        for ds_name, ds in train_dataset.dataset_dict.items():
            try:
                ...
                dict_file = os.path.join(config.STATIC_FITS_DIR, ds_name + '.npz')
                # dict_file2 = os.path.join(config.STATIC_FITS_DIR2, ds_name + '_fits.npy')
                a = np.load(dict_file)
                # print(a)
                # print(a['pose'].shape, a['betas'].shape)
                spin_fits = np.concatenate((a['pose'], a['betas']), axis = 1)
                print(spin_fits.shape)
                # print(np.load(dict_file2).shape)
                # self.fits_dict[ds_name] = torch.from_numpy(np.load(dict_file))
                self.fits_dict[ds_name] = torch.from_numpy(spin_fits)
