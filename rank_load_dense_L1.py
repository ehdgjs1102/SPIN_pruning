...
def load_densenet_model(model, oristate_dict):
    ...
    if orifilter_num != currentfilter_num:
        print('loading rank from: ' + str(cnt) )

        for index_i in range(orifilter_num):
            print(abs(oristate_dict[conv_weight_name][index_i]).sum())
        ...
        last_select_index = np.array(range(int(64)))
        ...
        for k in range(num):
            ...
            for num_conv in range(1,3):
                ...
                if orifilter_num != currentfilter_num:
                    ...
                    if last_select_index is not None:
                        print('(input&output)loading rank from: ' +  str(cnt) )
                        ...
                        print('(output)loading rank from: ' +  str(cnt) )
                        # print('select_index : ', select_index)
                        for index_i, i in enumerate(select_index):
                            state_dict[conv_weight_name][index_i] = oristate_dict[conv_weight_name][i]
                    ...
                    print('(input)loading rank from: ' +  str(cnt) )
                    # print('Last_select_index : ', last_select_index)
                    for index_j, j in enumerate(last_select_index):
                        for index_i in range(orifilter_num):
                    ...
                    print('(no_change)loading rank from: ' +  str(cnt) )
                    state_dict[conv_weight_name] = oriweight
                    for bn_part in bn_part_name:
                        state_dict[bn_name + bn_part] = oristate_dict[bn_name + bn_part]
...
checkpoint = torch.load('/home/urp10/SPIN/logs/densenet121_with_spinfits/checkpoints/2021_08_28-21_00_11.pt')
